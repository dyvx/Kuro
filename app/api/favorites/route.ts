...
function unauthorized() {
  return NextResponse.json({ error: "Sign in to use favorites." }, { status: 401 });
}
...
export async function GET() {
  ...
  try {
    const items = await store.listFavorites(userId);
    return NextResponse.json({ items });
  } catch (err) {
    console.error("[kuro] favorites GET failed:", err);
...
export async function POST(req: Request) {
  ...
  const store = getStore();
  try {
    await store.addFavorite(userId, {
    ...
    console.error("[kuro] favorites POST failed:", err);
...
export async function DELETE(req: Request) {
  ...
  const store = getStore();
  try {
    await store.removeFavorite(userId, animeId);
    return NextResponse.json({ ok: true });
  } catch (err) {
    console.error("[kuro] favorites DELETE failed:", err);
    return NextResponse.json({ error: "Could not update favorites." }, { status: 500 });
  }
}
